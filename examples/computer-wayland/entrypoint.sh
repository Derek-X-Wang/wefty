...
if [ -z "${WAYLAND_DISPLAY:-}" ]; then
  echo 'headless Wayland socket did not become ready within the 60 second startup budget' >&2
  exit 1
fi
...
supervise_edge() (
  ...
  while :; do
    ...
    rm -f "$socket"
    # wayvnc owns both RFB and WebSocket. The view role disables input inside
    # the server; no proxy or client-side filtering participates. The native
    # RFB parser records only roles whose input is enabled, making the input
    # receipt assertion-derived while keeping --disable-input load-bearing.
    record_input=$role
    if [ "$role" = view ] && [ -n "$input_flag" ]; then record_input=; fi
    if [ -n "$input_flag" ]; then
      WEFTY_WAYVNC_ROLE="$role" WEFTY_WAYVNC_RECORD_INPUT="$record_input" wayvnc -w "$input_flag" --output HEADLESS-1 -L info -R -S "$socket" "$address" "$port" &
    else
      # A transient seat supplies virtual pointer and keyboard capability even
      # when the headless backend deliberately has no physical input device.
      WEFTY_WAYVNC_ROLE="$role" WEFTY_WAYVNC_RECORD_INPUT="$record_input" wayvnc -w --transient-seat --output HEADLESS-1 -L info -R -S "$socket" "$address" "$port" &
    fi
    edge_pid=$!
    edge_status=0
...
if [ -z "$mutation" ]; then
  oracle_wait=20
  while [ "$oracle_wait" -gt 0 ] && { [ ! -s /tmp/wefty-computer/input-oracle.json ] || [ ! -s /tmp/wefty-computer/driver-state.json ]; }; do
    sleep 1
...
# Start the two screencopy sessions in order. On small arm64 hosts, opening
# both against the same headless output before either has completed a frame can
# stall their Wayland dispatch loops. The first real contract handshake is the
# barrier; no synthetic response or image-owned readiness claim participates.
if [ -z "$mutation" ]; then
...
# Give the contract edges the first scheduling turn. Chromium is optional
# desktop furniture and must not starve native RFB readiness.
