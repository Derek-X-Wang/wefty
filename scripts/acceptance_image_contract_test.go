...
func TestAcceptanceImageWorkflowContract(t *testing.T) {
	...
	assertFileContains(t, "../examples/computer-wayland/surface.py", "input-oracle.json", `"ready": False`, `INPUT["ready"] = True`, "def wefty_record_input", "Record only events delivered through Sway", `"wev"`, "wl_keyboard", "wl_pointer", "POINTER_MOTION", "focus_keyboard_oracle", `self.path not in ("/surface-ready", "/input")`, "agent-state-surface.json", "theme-surface.json", "os.replace")
	assertFileContains(t, "../examples/computer-wayland/oracle.html", "pointermove", "pointerdown", "pointerup", "window.outerHeight - window.innerHeight", "'/input'", "'/surface-ready'")
