...
func TestAcceptanceImageWorkflowContract(t *testing.T) {
	...
	assertFileContains(t, "../examples/computer-wayland/surface.py", "input-oracle.json", "def wefty_record_input", "Record only events delivered through Sway", `"wev"`, "wl_keyboard", "wl_pointer", "POINTER_MOTION", `self.path != "/surface-ready"`, "agent-state-surface.json", "theme-surface.json", "os.replace")
	...
	assertFileContains(t, "../examples/computer-wayland/sway-config", `title="^Wefty Wayland Computer$"] border none`, `app_id="wev"] border none`, "opacity set 0.0", "floating enable", "resize set width 1280 px height 720 px", "focus")
	assertFileNotContains(t, "../examples/computer-wayland/sway-config", "fullscreen enable")
