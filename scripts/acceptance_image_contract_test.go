...
func TestAcceptanceImageWorkflowContract(t *testing.T) {
	...
	assertFileContains(t, "../examples/computer-wayland/entrypoint.sh", "wayvnc -w", "--disable-input", "--class=chromium", "--app=http://127.0.0.1:18888/", "WEFTY_COMPUTER_VIEW_PORT", "WEFTY_COMPUTER_CONTROL_PORT", "surface-ready", "surface_wait=45", "within 45 seconds", "view-edge-ready", "control-edge-ready")
	assertFileContains(t, "../examples/computer-wayland/entrypoint-fixture.sh", "WEFTY_CONFORMANCE_MUTATION", "--class=chromium", "--app=http://127.0.0.1:18888/", "oracle_wait=45", "within 45 seconds", "wefty-view-proxy")
	...
	assertFileContains(t, "../examples/computer-wayland/surface.py", "input-oracle.json", `"ready": False`, `INPUT["ready"] = True`, "def wefty_record_input", "Record only events delivered through Sway", `"wev"`, "wl_keyboard", "wl_pointer", "POINTER_MOTION", "focus_input_observer", `'[app_id="wev"] focus'`, `self.path not in ("/surface-ready", "/input")`, "agent-state-surface.json", "theme-surface.json", "os.replace")
	assertFileContains(t, "../examples/computer-wayland/oracle.html", "pointermove", "pointerdown", "pointerup", "keydown", "window.outerHeight - window.innerHeight", "'/input'", "'/surface-ready'")
	assertFileNotContains(t, "../examples/computer-wayland/oracle.html", "keyup")
