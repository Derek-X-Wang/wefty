...
func TestAcceptanceImageWorkflowContract(t *testing.T) {
	...
	assertFileContains(t, "../runner/ocihelper/containerd_engine_realtiming_linux_test.go", "CodeImageUnavailable", "ImageFailureNetwork", "WEFTY_OCI_PROVISION_RECEIPT", `evidenceSource := os.Getenv("WEFTY_REALTIME_EVIDENCE_SOURCE")`, `registryEvidence := fmt.Sprintf("pull_from_empty=true\npull_import_digest_equal=true\nimage_pull_reference=%s\nimage_import_reference=%s\n", imagePullReference, imageImportReference)`, "pull_from_empty=NOT-RUN\\npull_from_empty_reason=pr-build: image not published")
