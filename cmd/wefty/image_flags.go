...
func (f *imageFlagSet) programAndTags(tags []string, class string) (*contract.ImageProgram, []string, error) {
	...
	if err := contract.ValidateImageProgram(*program, class); err != nil {
		return nil, nil, usageError(fmt.Sprintf("invalid image program: %v", err))
	}
	if err := contract.ValidatePinnedRouting(*program, l1NormalizedTags(tags)); err != nil {
		return nil, nil, usageError(fmt.Sprintf("invalid image routing: %v", err))
	}
...
func pinnedImageTags(tags []string, nodeID string, mounted bool) ([]string, error) {
	...
	return l1NormalizedTags(result), nil
}
