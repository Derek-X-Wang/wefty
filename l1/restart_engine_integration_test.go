...
func assertServiceCompletionClassification(t *testing.T) {
	...
	tests := []struct {
		...
		// Pins that the restart engine never reads log_evidence_incomplete: this row
		// classifies identically on the row above and must keep doing so.
		{name: "spontaneous signal with incomplete logs restarts", result: ProcessResult{Signal: "killed", TerminationCause: contract.TerminationCauseSpontaneous, LogEvidenceIncomplete: true}, wantJob: contract.JobQueued, wantAttempt: contract.AttemptFailed, wantStreak: 1, wantLifetimeRestarts: 1, wantBackoff: true, wantFailure: true},
