...
func TestWriteErrorPublishesTruthfulRetryability(t *testing.T) {
	tests := []struct {
		...
		{name: "stale admin policy", err: protocolError(contract.ErrorStalePolicyRevision, "stale admin policy"), wantStatus: http.StatusConflict, wantCode: contract.ErrorStalePolicyRevision},
		{name: "person identity required", err: protocolError(contract.ErrorPersonIdentityRequired, "missing person"), wantStatus: http.StatusUnauthorized, wantCode: contract.ErrorPersonIdentityRequired},
		{name: "admin required", err: protocolError(contract.ErrorAdminRequired, "not admin"), wantStatus: http.StatusForbidden, wantCode: contract.ErrorAdminRequired},
		{name: "invalid admin bootstrap", err: protocolError(contract.ErrorAdminBootstrapInvalid, "invalid bootstrap"), wantStatus: http.StatusForbidden, wantCode: contract.ErrorAdminBootstrapInvalid},
		{name: "closed admin bootstrap", err: protocolError(contract.ErrorAdminBootstrapClosed, "closed bootstrap"), wantStatus: http.StatusConflict, wantCode: contract.ErrorAdminBootstrapClosed},
