...
func TestOCIPrestartRuntimeLossRequeuesOnceAndExhaustsBudget(t *testing.T) {
	h := newIntegrationHarnessWithOptions(t, StoreOptions{PrestartInfrastructureBudget: 1500 * time.Millisecond}, map[string]NodePolicy{
...
func TestOCIServiceRuntimeFailureClassifierDefaultsTerminal(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "known infrastructure", code: contract.RuntimeFailureUnavailable, want: contract.JobQueued},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if err != nil || completed.State != test.want || completed.RestartStreak != 0 {
