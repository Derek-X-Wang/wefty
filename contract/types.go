...
const (
	...
	// DefaultResultRetention is how long a finished run's handoff directory --
	// its results -- is kept on the node that produced it. It is a contract
	// value rather than an agent flag because a person reading `wefty inspect`
	// has to know when their results stop existing, and the only honest way to
	// tell them without asking the node is for the rule to be the same
	// everywhere. A node configured otherwise is free to be, and its own sweep
	// is what actually decides; what this constant fixes is what everything
	// else may assume.
