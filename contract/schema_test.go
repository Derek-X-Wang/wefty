...
func TestJobSpecSchemaAndGoValidationAgree(t *testing.T) {
	...
	cases := map[string]validatorCase{
		...
		"control driver mount shadow":              {raw: `{"schema_version":1,"dispatch_key":"oci:control-shadow","kind":"oci","class":"one-shot","routing_tags":["wefty:node:node-a"],"execution":{"oci":{"image":{"reference":"alpine:latest"},"mounts":[{"node_path":"/srv/input","container_path":"/wefty/control/driver.json"}]}}}`, wantValid: false},
...
func TestImageProgramSchemaAndGoValidationAgree(t *testing.T) {
	...
	cases := map[string]validatorCase{
		...
		"reserved control mount child":  {raw: `{"reference":"alpine:latest","mounts":[{"node_path":"/srv/input","container_path":"/wefty/control/driver.json"}]}`, wantValid: false},
