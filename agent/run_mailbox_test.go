...
func mailboxClaim(runID, handoff, params string) l1.Claim {
	return l1.Claim{
		// Mailbox eligibility follows L1's classification of the submitter, so
		// the fixture carries the bit a real L3 dispatch always claims with.
...
func TestUnpublishedMailboxEvidenceRetainsTheHandoffOnSuccess(t *testing.T) {
	...
	// Published stays on the record because part 2 and #494 need it; in part 1
	// it decides nothing, because there is no node budget to order.
}
