...
If its bounded log flush or upload expires, unacknowledged spool events remain
available for later durable recovery while the completion records
`log_evidence_incomplete`; the deadline does not replace an observed exit or
signal with `output_error`.
