...
A copy whose removal L1 has already acknowledged is never one of those
refusals. The bound node keeps its own durable record of the copies L1
accepted as absent, matched on copy and Backup identity, Storage identity and
generation, bound node, root instance, operation revision and cleanup fence,
and skips them when a directive list built before the acknowledgement names
them again: no second deletion call and no second receipt. L1 accepting a
renewed positive-absence receipt for an already-removed copy is the second
guard behind that, so a replayed acknowledgement cannot extend the streak
either.
...
exactly the absence L1 recorded, and it returns the pruned Backup without
mutation while the first accepted receipt stays the record. The helper mints a
fresh receipt identity on every deletion call, so a differing receipt identity
is renewed evidence, not conflicting authority. Only reusing one idempotency
key with a different body is refused, as `idempotency_conflict`; a receipt that
disagrees about Backup, copy, Computer, Storage identity or generation, bound
node, root instance, operation revision or cleanup fence is still `conflict`.
The same holds for a superseded create's planned copy and for a restore
predecessor kept as a Backup.
