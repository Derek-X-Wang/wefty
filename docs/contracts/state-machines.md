...
the same value accepted by Fabric dialing, not a rewritten or provider-aware
alias. The CLI reads that file, sends the bearer only to the fixed sideband
path on the same front door and same Fabric identity, and never prints or
persists its contents itself. The sideband remains the authority: a copied
file from another person, device, ended session, or Computer fails closed, and
neither a CLI flag nor URL selects the control backend directly.
