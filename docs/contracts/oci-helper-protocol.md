...
| `InventoryRemoval` | Session-authorized current inventory for legacy removal reconstruction. Ordinary results require exact observed attempt authority. Under serialization with the live-attempt registry and the generation attachment lock, a Computer generation may return Storage-only only when the current scan found zero matching attempt authorities, its disk is prepared by a positive durable copy/reset receipt bound to the Node and managed-root instance, and it has no attachment lineage, loop, mount, pending attachment, or retirement. The result is bound to the current Node, boot, Job, removal generation, cleanup fence, and exact disk identity; its runtime identifiers remain empty and it never invents runtime resource rows. |
