...
**before** it sweeps, and the tripping generation records the trip in the same
fsynced write as the failure that caused it. **After a trip, at most one
startup sweep runs per window, per node, until one succeeds.** A generation
that inherits a tripped ledger keeps serving, reports the bound and its next
attempt time in every authority-free handshake preface, and refuses every
session admission with `startup_bound_tripped`. Holding the listener is what
ends the relaunches -- systemd activates nothing while a live process owns the
socket -- so this generation must not exit. It re-attempts the barrier one
window after the last failure, and immediately at launch if that window has
already elapsed; a re-attempt that fails extends the streak in the ledger and
schedules the next one a window later. Only a barrier that succeeds clears the
ledger. The ledger is never consumed by a refusal: on a native Linux node
nothing restarts the helper unit -- there is no Lima repair, and
`wefty node oci start` reaches the same refusing process -- so a bound held
only in one process's memory would outlive the repair that cleared the denial.
Re-arming is what lets such a node recover with no restart at all. The refusal
is a completed handshake without admission, so the agent's boot barrier reports
it as a stalled handshake at the first dial rather than after a whole takeover
window, and the unchanged bounded repair proceeds. The node doctor names the
tripped bound as `oci_helper_startup_bound_tripped` from that handshake, with
the time the handshake was read, because a helper that admits no session serves
no diagnostic read and a bound is a fact about one reading. A ledger
...
A reused handoff volume is **live from the moment its attempt is admitted**,
not from the moment its task exists. `Run` publishes the durable ownership
record and unlinks the prior attempt's terminal receipt in one hold of the
retention lock, before the container and task are created and before the volume
is mounted. The two have to move together: liveness is "owned *and* carrying no
valid receipt", because a finalized owner whose release was deferred must not
pin its volume forever -- so publishing ownership first and superseding the
receipt afterwards left the volume reading as idle for the whole of container
and task creation, which is exactly the interval a budget eviction arrives in.

`handoff_volume_live` is the refusal a handoff-volume deletion gets when an
attempt still owns that volume. It is replayable contention and not an engine
failure: nothing was detached, nothing was freed, and the identical request
succeeds once the owner finishes. The caller it exists for is the node agent's
budget eviction, which selects from an inventory snapshot and can therefore
arrive after a rerun has claimed the volume it chose; the helper decides it
under the same lock `Run` registers ownership and supersedes the prior receipt
under, because a check anywhere else has a window after it, and the thing in
...
| --- | --- |
| `EnsureImage` | Session-authorized, typed progress/result stream on a dedicated connection. The agent supplies the canonical platform retained from the successful probe for this helper generation; manifest selection and image singleflight are keyed by it. That canonical platform is containerd's normal form (`platforms.Normalize`, under which arm64 carries no variant), which is also the form every piece of helper image evidence comes back in; both sides compare platforms in that form and in no other, so one architecture can never have two spellings that fail to equal each other. The sole offline-bootstrap exception is the clean-cache `node load-image` archive import that must precede that probe: it may use the current helper diagnostic OS/architecture reduced to the same normal form, but it does not retain or promote that diagnostic fact as probe evidence. Every registry delivery, binding pin, and other caller remains gated on the probe-retained canonical platform, and later probe evidence must select the same archived platform digest. Registry mode resolves only a public reference, pins the returned top-level digest, pulls into the fixed namespace, and unpacks that platform. Archive mode receives an OCI-layout tar stream, recomputes every blob digest, validates descriptor sizes and reachability, admits exactly that platform, and imports/unpacks it. Both modes return the same complete image evidence used by `Run`, including top-level/platform digests, platform, runtime handler, and snapshotter; no containerd type, private registry credential, or retry policy crosses the boundary. |
...
| `InventoryHandoffVolumes` | Session-authorized, and deliberately not attempt-authorized: it spans every handoff volume this node still holds, and the attempts that produced them are long gone, so no attempt's fence could stand for them. The request carries a page cursor and nothing else, and cannot carry attempt authority -- a body that attaches one is refused as an unknown field; a cursor longer than one fixed scan identity, separator and volume name is refused as an invalid request. It returns, for each volume, the deterministic directory name, the terminal time and whether that came from a helper-owned receipt, logical, deduped and **charged** bytes, entry count, whether a live attempt is still writing there, whether the byte and entry figures are a floor rather than a measurement, and a bounded set of per-volume anomaly tokens from a closed vocabulary. Charged bytes are the node-budget unit and cross the wire rather than being derived on the agent's side: every entry contributes the larger of its deduplicated logical bytes and 4 KiB, which is the identical rule the agent applies to its own handoff root, and no function of a volume total and an entry count reproduces a per-entry floor. **It is a page of one scan, and the pages of one listing are one consistent view.** A request with no cursor runs the prologue, reads the generation, enumerates and measures the whole root once under the existing entry, open and row budgets, then validates that the generation did not move before caching and serving the first page. A changed root is rescanned internally up to the fixed bound; exhausting it returns an incomplete scan. Every later page is cut from that one validated measurement. That is what makes two things true at once. A volume created behind the cursor cannot be shown by no call at all, because pages are not independently measured; and one `(device, inode)` map spans the whole root, so a file two volumes hard-link is charged once however far apart those volumes sort. Per-page dedup would have charged it once per page, and a budget that overstates a node gives up results it did not have to -- which is destructive, not merely inaccurate.

The helper keeps a monotonic generation for the root's shape, bumped at every successful visible volume creation, detach, receipt publication and receipt removal, including a mutation whose later durability or cleanup step fails, and at every change to attempt ownership -- published, released or quarantined -- because liveness is "owned *and* carrying no valid receipt", so an owner appearing or disappearing changes what a listing would say just as surely as a volume does. Every bump is taken under the retention lock, in the same hold as the change it describes, which is what makes the two one event to a reader sampling the generation under that same lock -- an ownership release takes the retention lock before it takes the ownership lock, the order publication already uses, rather than bumping afterwards and leaving an instant in which the owner is gone and the counter has not moved. A change that lands and then fails a later durability step bumps too: a record the filesystem is holding and a generation that never moved is a listing told nothing happened, and a bump for a change that did not land costs a listing one restart. A cached scan the generation has moved past is dropped rather than held until some later listing replaces it, so an abandoned listing does not pin a whole root's rows for the life of the session. Every page carries the generation of the scan it came from, so a reader can assert that what it is stitching together is one root. The opaque cursor is `<scan-id>:<volume-name>`; a cursored request whose identity is no longer the session's cached scan, or whose scan generation no longer matches the current generation, is answered `restart=true` with no rows and no cursor. A second first-page request may replace the session cache, but an older continuation can therefore never splice onto it. The reader starts over, bounded -- a root that changes faster than it can be listed produces an incomplete read, which withholds that one decision rather than blocking the node.

`exhausted` says this page stopped before the end of the scan and `next` is the cursor to resume from; a page that reached the end reports `exhausted=false`, and only that answer means a reader has seen everything the node holds. A scan that could not reach the end of the root itself reports its last page `exhausted=true` with no cursor, which is a reader being told it has not seen everything and has no way to ask for the rest. A flag with no cursor was not enough on its own: the tail of a large root was unreadable by any number of calls. The response is bounded in encoded bytes rather than rows, because a frame past the wire limit is refused by the transport and read by the client as a lost session -- accounting must never cost a node its runtime -- and the anomaly vocabulary is closed for the same reason: what a node reports cannot grow with what a workload wrote. No owner key crosses this boundary in either direction: the agent derives the names of its own runs with the same deterministic mapping, so a reported name it cannot derive is precisely the crash-residue signal. An unreadable, invalid or mismatched receipt is a per-volume anomaly with `terminal_known=false`, never a failed call and never a new error code; a refusal of this read is scoped to it and is not runtime-loss evidence. Before reporting, it repairs: any handoff volume whose receipt path is absent, and which a fresh in-memory and durable ownership snapshot says no live attempt holds, is stamped with one, best-effort and atomically create-only, so a receipt the filesystem refused is not left waiting for another boot sweep. Repair revalidates the measured directory identity at publication and cannot replace a receipt concurrently written by `Delete`. It **deletes no volume and no receipt**, and re-dates no terminal time, because expiry belongs to the sweep and budget eviction to the agent, and a read must never cost a node a run's results. It does finish freeing trees an earlier authorized deletion detached: those are neither a volume nor a receipt but bytes whose removal was already authorized and interrupted, and a detached name is invisible to this inventory, to expiry and to repair, so leaving them to the next boot sweep would hide them for as long as the session lived. The response reports how many such frees are still pending, since their bytes are on the node and in no volume's figures. |
...
intact, and sweep removes only expired direct children with the deterministic
handoff prefix. That age is the contract's result-retention window (7 days),
not a retry window: a handoff volume holds the run's results and is retained
on every outcome. Attempt and namespace quiescence therefore project only
unexpired handoff volumes (the retained bindings) out of their absence
decision.

Retention runs from a helper-owned terminal receipt, never from the volume's
mtime. The handoff volume is mounted read-write into the container and a uid-0
workload owns it, so its mtime is a timestamp the workload writes -- by
`utimensat` or by merely creating a file. The helper therefore records the
terminal time itself, in a separate durable root `handoffs-state/` that the
container is never given a path to, as one versioned, fsynced,
write-then-rename JSON receipt per volume named
`wefty-handoff-volume-<hex>.retention` carrying the volume's device and inode,
the terminal time, and the bytes and entries measured at that moment. That root
is deliberately separate rather than a sibling inside `handoffs/`, because
every scan of the handoff root matches on the deterministic volume prefix and
a sibling carrying it would be read as a volume. The receipt is evidence and
the no-follow directory descriptor is authority, exactly as for a service data
owner record: a receipt whose device and inode no longer match the directory
standing at that name is refused rather than believed.
...
volume for reuse publishes durable ownership and registers the in-memory owner
before superseding the previous attempt's receipt, all receipt-sensitive steps
serialized against accounting repair: a run that has not finished has no
terminal time, and inheriting one made a rerun's own `Delete`
...
`terminal_known=false` and the mtime as a labelled fallback: it may be counted
...
ask to have removed, and is left to the sweep. **Whether an attempt still owns
the volume is checked here**, under the same lock `Run` admits ownership and
supersedes the prior receipt under, and a volume that is owned is refused with
`handoff_volume_live` -- nothing detached, nothing freed, the same request
replayable once the owner finishes. Anywhere else the check would have a
window after it, and the thing in that window is a rerun that claimed the
volume between the agent's inventory read and this call; the sweep's own
liveness filter has the same shape and now passes through the same guard. The
agent also never selects a volume it saw reported `live` or one its own
admission record says it is running, so this refusal is the last of three
...
are: the boot receipt's "observed = runtime residue union durable retained"
invariant has to keep holding over every durable thing the helper writes, and
a class that is neither observed nor projected is a class nothing verifies. A
receipt is projected durable-retained exactly while its paired volume is, and
is runtime residue otherwise.
...
Clone has exactly this one typed refusal code. Every other clone failure leaves
the copy's integrity in doubt: an engine failure, a deferred resume, or a
quarantined generation, which is reserved for a copy whose bytes cannot be
trusted.

For a Custody import or a clone, typed helper runtime loss during
`CopyComputerStorage` becomes an exact-generation
`computer_storage_preparation_interrupted` observation; startup recovery also
carries exact-generation `computer_storage_resume_deferred` or
`computer_storage_quarantined` results.
...
time; an identity mismatch never attaches to the copy it claims. L1 records the
preparation idempotency key and body hash, accepts identical replay, and rejects
an older helper generation or timestamp. Restore has no such path at all: its
destination is a live Computer whose predecessor still owns the bytes, so it
holds no never-attached generation that a preparation outcome could close.

For an import this evidence grants no publication
...
generation without eligible preparation evidence returns `no_storage_evidence` with no attempts; it does not
assert runtime absence. The Job-scoped runtime scan remains the sole runtime
authority, and reconstruction refuses only when neither scan proves authority.
