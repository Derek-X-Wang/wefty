...
| `Signal` | Exact live attempt and only enumerated `TERM` or `KILL`. A containerd `NotFound` after authorization is the closed `task already terminated` mechanics fact: the helper returns success without recording delivery of that signal, and `Watch` remains authoritative for the terminal arm. This race is not `engine_failure` or runtime-loss evidence. |
...
runtime-loss receipt or reset the verified helper session; genuine loss still
uses the ordinary replacement-sweep receipt above.
