...
The helper keeps a monotonic generation for the root's shape, bumped at every successful visible volume creation, detach, receipt publication and receipt removal, including a mutation whose later durability or cleanup step fails, and at every change to attempt ownership -- published, released or quarantined -- because liveness is "owned *and* carrying no valid receipt", so an owner appearing or disappearing changes what a listing would say just as surely as a volume does. Every bump but one is taken under the retention lock, which is what makes the change and the bump one event to a reader sampling the generation under that same lock; an ownership release bumps just after its own lock is released, because publication takes the retention lock and then the ownership lock and the reverse order would be an inversion. A cached scan the generation has moved past is dropped rather than held until some later listing replaces it, so an abandoned listing does not pin a whole root's rows for the life of the session. Every page carries the generation of the scan it came from, so a reader can assert that what it is stitching together is one root. The opaque cursor is `<scan-id>:<volume-name>`; a cursored request whose identity is no longer the session's cached scan, or whose scan generation no longer matches the current generation, is answered `restart=true` with no rows and no cursor. A second first-page request may replace the session cache, but an older continuation can therefore never splice onto it. The reader starts over, bounded -- a root that changes faster than it can be listed produces an incomplete read, which withholds that one decision rather than blocking the node.
