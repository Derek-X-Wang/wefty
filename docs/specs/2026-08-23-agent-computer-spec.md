...
Each Computer has one non-transferable `storage_id` and one current, monotonically identified Storage generation. The generation is a fully preallocated loop-backed ext4 image under the helper-owned guest-native root, with zero reserved blocks; all tenant-writable paths are charged to that budget through a read-only root plus bounded tmpfs or equivalent accounting. Allocation itself is the disk admission gate, and `ENOSPC` before `Started` leaves no partial file, loop, mount, or manifest. The allocation is also held: the loop device is attached with block discard refused, and every attach re-asserts the full allocation, so neither a guest trim nor the filesystem's own deallocating write-zeroes can retire bytes the node is already charged for. A budget that the host can no longer honour refuses that attach, typed and retryable; it never retires the Computer. (#120 agent-computer; #122 agent-computer; #442)
...
A Custody export event commits before the first external byte. Partial or complete export permanently taints that provenance branch and descendants; removal closes a still-planned export directive without retracting that event, and typed failure or dead-Node abort closes `exporting`. An `operator_attested_deleted` event may be recorded but never upgrades removal. The portable `custody.json` contains the sanitized Job specification, immutable manifest/digest inputs, and Storage provenance, so import survives loss of the exporting L1 database. Import is directed to a current operator-selected Node (or the discoverable Node holding the path), never the export's frozen root instance; it atomically reserves new Computer/storage identities with no grants in the shared Storage-copy ledger, verifies manifest and bytes, performs clone-style OS rekeying, and inherits custody taint. Failed import positively cleans and attests staging absence and releases its reservation. Pre-removal exports may return only as new Computers. (#121 agent-computer)
