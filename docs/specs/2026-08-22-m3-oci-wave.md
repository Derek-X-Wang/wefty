...
This is paste-ready issue text, not implementation. Titles are stable dependency keys: copy every `Blocked by:` title exactly when creating GitHub edges. The cache ceiling is ratified at 16 GiB; no ticket is gated on an owner call.
...
> Default ceiling: 16 GiB, owner-ratified 2026-08-22.
...
- VM sizing flags `--vm-memory/--vm-cpus/--vm-disk` with the spec defaults (25% host RAM capped at 4 GiB; 4 vCPU capped at half the cores; 32 GiB) — see spec §8 "VM sizing".
...
No new service-data byte deletion or proof-gated absence attestation. The
existing spool, managed-resource, image-pin, acknowledgement, and completion
sequence remains in force after the manifest records quiescence.
