...
## doctor-code-oci-retained-results-not-read

Meaning: the node agent has not completed a retained-results accounting pass, or the doctor was built without a reader for one, so this node cannot say what its retained results are holding. It is not a statement that the node is holding nothing: a node that has not measured and a node that is empty report the same zeroes, and this code exists so the two are never confused. Evidence: the agent log, which prints one `agent: retained results on this node:` line per collection — at startup and hourly. First action: confirm the agent is running and has completed a collection; a node that has been up for more than an hour with no such line has a collector that is not running. Escalation: attach the agent log around startup and the doctor report.
...
Meaning: the node's retained results were measured, and some entries under the handoff root are not this agent's: no record names them. A directory with no agent record is never measured and never removed, however full the node gets, so these are bytes the node is holding and cannot give up on its own. Ordinary causes are a directory a previous agent left without an ownership marker, a directory another tool wrote under the handoff root, and a subtree that stopped being the directory the accounting pass was walking. Evidence: `wefty --json node oci doctor` for the count, and the agent log, whose per-pass line names the same figure and whose adoption lines name every directory startup refused to claim and why. First action: list the handoff root and decide, per directory, whether it is yours to remove; the agent will not remove it for you. Escalation: attach the doctor report, the handoff root listing, and the agent's startup adoption lines.

## retained-results-record-names

Meaning: this agent version files a run's retention record under a name that cannot collide with another run's. Older versions folded every character outside `[A-Za-z0-9_-]` to `_` and cut the name at 96 bytes, so `run.live` and `run_live` shared one file, as did any two runs agreeing on their first 96 characters. Records written under the old names are still read, so upgrading needs no migration. **Downgrading does:** an agent older than this one cannot find a record written under the new name, so a run whose ID contains a `.` or is longer than 96 characters keeps its retained results on the node, unexpired, until the agent is upgraded again. Nothing is deleted and nothing is lost; the node holds more than it should until then. Evidence: the agent log's per-pass `agent: retained results on this node:` line, whose unrecorded count rises as those directories stop being named by any record the older agent can read. First action: upgrade the agent back. Escalation: attach the handoff root listing and the agent's state directory listing.
